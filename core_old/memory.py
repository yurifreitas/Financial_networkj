...
from core.hyperparams import SAVE_PATH, MEMORIA_MAX, EPSILON_INICIAL
...
class RingReplay:
    ...
    def append(self, s, a_idx, r, sn, d, y_ret):
        i = self.idx
        # Adiciona uma nova transição ao buffer
        self.s[i], self.a[i, 0], self.r[i], self.sn[i], self.d[i], self.y[i] = s, a_idx, r, sn, d, y_ret
        self.p[i] = abs(r) + 1e-3  # Prioridades baseadas na recompensa

        # Incrementa o índice
        self.idx = (self.idx + 1) % self.cap

        # Marca o buffer como cheio quando o índice retorna a 0
        if self.idx == 0:
            self.full = True

    def __len__(self):
        # Retorna o comprimento do buffer, se estiver cheio retorna a capacidade total
